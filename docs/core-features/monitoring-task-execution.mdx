...
When you start a task attempt, Kira Code orchestrates several steps to create an isolated environment and execute your task:
...
Kira Code uses Git worktrees to create isolated environments for each task attempt. These environments are ephemeral and automatically cleaned up after execution completes.
...
Kira Code generates commit messages based on the last message sent by the agent. These automated messages may not always be the most descriptive.
