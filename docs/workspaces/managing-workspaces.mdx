...
|----------|-----------------|
| **macOS** | `/var/folders/.../T/kira-code/worktrees` (system temp) |
| **Linux** | `/var/tmp/kira-code/worktrees` |
| **Windows** | `%TEMP%\kira-code\worktrees` |

You can configure a custom location in **Settings → General → Workspace Directory**. When set, worktrees are stored in `{your-path}/.kira-code-workspaces`.
...
<Tab title="macOS">
```bash
du -sh $TMPDIR/kira-code/worktrees
```
</Tab>
<Tab title="Linux">
```bash
du -sh /var/tmp/kira-code/worktrees
```
</Tab>
<Tab title="Windows">
```powershell
Get-ChildItem "$env:TEMP\kira-code\worktrees" -Recurse | Measure-Object -Property Length -Sum
```
</Tab>
</Tabs>
...
If you manually delete a worktree directory (e.g., using `rm -rf` in terminal), Kira Code will **automatically recreate it** the next time you open that workspace. The worktree is recreated from the branch's last commit.
...
Kira Code automatically cleans up "orphaned" worktrees on startup - these are worktree directories that no longer have a matching workspace in the database (e.g., if the app crashed during deletion).
