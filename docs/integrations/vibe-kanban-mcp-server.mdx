---
title: "Kira Code MCP Server"
description: "Configure the Kira Code MCP server"
---

Kira Code exposes a local MCP (Model Context Protocol) server, allowing you to manage organisations, projects, issues, workspaces, and repositories from external MCP clients like Claude Desktop, Raycast, or even coding agents running within Kira Code itself.

<Note>
This page covers connecting **external MCP clients** to Kira Code's MCP server. For configuring MCP servers **within** Kira Code for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
Kira Code's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the Kira Code MCP server to any [supported coding agent](/supported-coding-agents) **within** Kira Code.

1. In Kira Code Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the Kira Code card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.png" alt="MCP Servers configuration page showing how to add Kira Code MCP server" />
</Frame>
...
```json
{
  "mcpServers": {
    "vibe_kanban": {
      "command": "npx",
      "args": ["-y", "kira-code@latest", "--mcp"]
    }
  }
}
```

`--mcp` launches the local MCP stdio server. Any additional arguments after `--mcp` are passed through to the `kira-code-mcp` binary.

## Available MCP Tools

The Kira Code MCP server provides tools for managing organisations, projects, issues, workspaces, and task execution.
...
3. **Automatic Issue Generation**: Your MCP client will use the Kira Code MCP server to automatically create structured issues in your project
...
Your MCP client will use the `create_issue` tool to break this down into individual issues and add them to your Kira Code project automatically.
...
### Internal Coding Agents (Within Kira Code)

A powerful workflow involves using coding agents within Kira Code that are also connected to the Kira Code MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual issues directly in the Kira Code interface
...
Raycast is a popular MCP client that can connect to Kira Code's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding Kira Code server" />
</Frame>

Configure the Kira Code MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the Kira Code MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>
